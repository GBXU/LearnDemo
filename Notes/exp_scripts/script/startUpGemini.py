...
class experi_handler():
    def __init__(self, testName, dcList,dbFile, userName, password, path, debug, isAmazon, nodeFile):
        ...
        self.txmud_outputdir = path + "/output/txmud"
        ...
        #define jar file
        jarFilePath = self.txmud_binary
        self.coordinatorJar = jarFilePath+"/coordinator-big.jar"
        self.storageshimJar = jarFilePath+"/storageshim-big.jar"
        self.jsqlparserJar = self.txmud_src_path + "/lib/jsqlparser.jar"
        self.nettyJar = self.txmud_src_path + "/lib/netty-3.2.1.Final.jar"
        self.logJar = "/var/tmp/workspace/MPI-txmud-java/lib/log4j-1.2.15.jar"
        
        #tpcw jars
        self.jdbcTxMudJar = self.txmud_binary + "/jars/jdbctxmud.jar"
    ...
    def prepareCode(self):
        ...
        #jarFilePath = svn_checkout_compile(self.debug,"chengli",self.pwd)
    ...
    def configure_proxy(self, ssc,dcId, proxyId): 
        ...
        command = "cd "+self.proxy_root_path + " && ant clean undeploy dist deploy -Dbackend=txmud -Dtotalproxy="+str(self.wpNum) +" -DdcCount="+str(self.dcNum)
