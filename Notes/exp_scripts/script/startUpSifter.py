...
class experi_handler():
    def __init__(self, testName, dcList,dbFile, userName, password, path, debug, isAmazon, nodeFile):
        ...
        self.txmud_src_path = "/home/"+self.userName+"/newJava"
        ...
        self.nettyJar = ""
        self.logJar = ""
        self.rubis_root_path = "/home/"+self.userName+"/newJava/src/applications/RUBiStxmud"
        ...
        #define jar file
        jarFilePath = self.sifter_root_path + "/dist/exec/"
        self.coordinatorJar = jarFilePath+"coordinator-big.jar"
        self.storageshimJar = jarFilePath+"storageshim-big.jar"
        self.sifterJar = self.sifter_root_path + "/bin/lib/georeplication.jar"
        self.jsqlparserJar = self.sifter_root_path + "/lib/jsqlparser.jar"
        self.nettyJar = self.sifter_root_path + "/lib/netty-3.2.1.Final.jar"
        self.logJar = "/var/tmp/workspace/MPI-txmud-java/lib/log4j-1.2.15.jar"
        
        #tpcw jars
        self.jdbcTxMudJar = "/var/tmp/chengli/output/txmud/dist/jars/jdbctxmud.jar"
        
        self.warmUpTime = 60000
    ...
    def initiateDatacenters(self):
        ...
        self.logicalClock = getLogical_clock(self.dcNum)
    ...
    def prepareCode(self):
        ...
        if not os.path.exists(self.sifter_root_path):
            print "Sifter code doesn't exist, please download"
            sys.exit()
        
        #compile the code here
        command = "cd " + self.sifter_root_path + " && ant clean && ant"
        print command
        os.system(command)
    ...
    def clean_database(self):
        ...
        for s in self.dcs:
            for x in range(0, s.dbNum):
                ...
                stopMySQLServer(ssc)
                resetDatabase(ssc, self.isAmazon, self.dcNum)
    ...
    def copyCodeToRemote(self):
        ...
        for dc in self.dcs:
            ...
            if dc.coorNum > 0:
                coorPrefix = "coor-"+str(dc.dcId)
                command = "rm -rf " + self.top_path + "/" + self.coordinatorJar.split("/")[-1]
                self.sshList[coorPrefix].run_command(command)
                self.sshList[coorPrefix].put(self.top_path+"/"+self.coordinatorJar.split("/")[-1],self.coordinatorJar)

            #storage
            storagePrefix = "sshim-"+str(dc.dcId)
            for k, v in dc.storageshims.items():
                command = "rm -rf " + self.top_path + "/" + self.storageshimJar.split("/")[-1]
                self.sshList[storagePrefix+"-"+str(k)].run_command(command)
                print "copy jar file  to storage node", dc.dcId, k
                self.sshList[storagePrefix+"-"+str(k)].put(self.top_path+"/"+self.storageshimJar.split("/")[-1],self.storageshimJar)
    ...
    def start_coordinator(self, dcObj):
        ...
        blueTokenNum = int(self.blueToken)
        print "blueTokenNum", blueTokenNum
        command = "cd "+self.top_path + " && " + "nohup java -Xms2000m -Xmx4000m -XX:+UseParNewGC -XX:+CMSParallelRemarkEnabled -XX:+UseConcMarkSweepGC -jar " + self.coordinatorJar.split("/")[-1] + " "
        command += self.dcFile+ " " + str(dcId)+ " " + str(dcId) + " " + str(self.coorThdCount) + " " + str(blueTokenNum) + " " + "true " + str(self.timeout) + " " + str(self.bluequietTime) + " " + str(0.5)
    ...
    def start_storageshim(self, dcObj, ssId): #start command needs to be modified
        ...
        command = "cd "+self.top_path + " && " + "nohup java -Xms2000m -Xmx4000m -jar " + self.storageshimJar.split("/")[-1] + " "
        command += " "+ self.dcFile + " "+self.dbFile +" " + str(dcId) + " " + str(ssId) + " " + str(self.sshimThdCount) + " true " + str(scratchpadNum)
    ...
    def deploy_tomcat(self, ssc):
        ...
        if ssc.rexists(self.tomcat_top_path) == False:
            command = "wget -P "+self.top_path+" -c ftp://ftp.de.cw.net/pub/FreeBSD/ports/distfiles/apache-tomcat-6.0.35.tar.gz"
            ssc.run_command(command)
    ...
    def deploy_mysql_driver(self, ssc):
        ...
        if ssc.rexists(self.tomcat_lib_path+"/mysql-connector-java-5.1.17-bin.jar") == False:
            command = "wget -P "+self.top_path+" -c http://mirrors.dedipower.com/www.mysql.com/Downloads/Connector-J/mysql-connector-java-5.1.17.zip"
            ssc.run_command(command)
    ...
    def configure_proxy(self, ssc,dcId, proxyId): 
        ...
        command = "cd "+self.proxy_root_path + " && ant clean undeploy dist deploy -Dbackend=sifter -Dtotalproxy="+str(self.wpNum) +" -DdcCount="+str(self.dcNum)
